...
class ProductsClass(Resource):
    ...
    def get(self):
        ...
        for product in products:
            response_body.append(product.to_dict(only=('id', 'title', 'price', 'length', 'width', 'height', 'description', 'quantity', 'img_url', 'subject', 'artist.name')))

        return make_response(jsonify(response_body), 200)
        
    def post(self):
        json_data = request.get_json()
        new_product = Product(artist_id=json_data.get('artist_id'), category_id=json_data.get('category_id'), title=json_data.get('title'), price=json_data.get('price'), length=json_data.get('length'), width=json_data.get('width'), height=json_data.get('height'), description=json_data.get('description'), quantity=json_data.get('quantity'), img_url=json_data.get('img_url'), subject=json_data.get('subject'))
